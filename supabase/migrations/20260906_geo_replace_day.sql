-- ════════════════════════════════════════════════════════════════════════
-- geo_replace_day: the ONE write path for automatic time and mileage.
--
-- Owner spec 2026-09-02 (see js/geo-derive.js header). The phone derives a
-- whole day from the CoreMotion tape and its GPS fixes, and hands the result
-- here. This function replaces that person's automatic rows for that day, in
-- one transaction, and refuses the set if any two rows in it overlap. It is
-- the database's half of the missing invariant: one person, one timeline.
--
-- Why a function and not a constraint on the tables: an EXCLUDE constraint
-- cannot be added while the tables still hold the overlapping rows the old
-- design wrote (and it cannot be added NOT VALID). Additive per CLAUDE.md
-- 3.1: nothing production reads changes shape; only who is allowed to write
-- automatic rows changes, and that change is in the client.
--
-- What is preserved, always:
--   * job_time_entries rows a person made (source = 'manual') or corrected
--     by hand (client_key like 'fixed-%'). A derived row that overlaps one of
--     those is dropped, not written: a human's answer outranks the tape.
--   * td_mileage rows that are not GPS legs (hand-typed trips), and on a GPS
--     leg that is being re-derived, the attributes a person set on it
--     (vehicle, purpose, notes, receipt state).
--
-- Idempotent by key: the same derived set upserts onto the same rows and
-- soft-deletes nothing, so a boot rebuild that changes nothing costs nothing
-- and never churns ids under the reader.
-- ════════════════════════════════════════════════════════════════════════
