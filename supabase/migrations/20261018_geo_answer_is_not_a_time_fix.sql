-- ── AN ANSWER IS NOT A TIME CORRECTION (owner 2026-09-16) ─────────────────
--
-- "Go look at Jack and tell me why we still have manual time ahead of Laurie."
--
-- Because answering a visit was being recorded as if he had hand-corrected its
-- TIMES. geo_answer_visit stamped fixed_at, and step 3 below reads fixed_at as
-- "the person's times win and everything else is refreshed from the evidence".
--
-- His 15 September, exactly:
--
--   regionEnter Laurie 07:59:25   the OS saw him arrive
--   motion walking      08:30:53   CoreMotion's first flip after the app died
--   drive row  07:54:54 - 07:59:25  re-derived, correct (rule 21)
--   Laurie row 08:30:53 - 09:07:18  FROZEN, because he answered it at 01:34pm
--
-- Rule 21 shortened the drive to the crossing on every rebuild, and the dwell
-- could never follow it, because his ANSWER was holding the old start in place.
-- The thirty-one minutes between them is a hole, and the Time Log fills a hole
-- under a running clock with Manual time. Nothing was wrong with the deriver;
-- the writer was refusing to take its word for a number nobody had set.
--
-- The two facts are now two columns. `answered_at` says a person settled what
-- this stop MEANT, which a rebuild must never talk over. `fixed_at` still says
-- a person set its TIMES, which is the only thing that should freeze a clock.
-- Both keep a row out of the sweep: what a person touched is never retired.
--
-- Rows answered before today keep their fixed_at and so keep their times. That
-- is deliberate: nothing here can tell those apart from a real hand edit, and
-- guessing would move times nobody asked to move. They re-grade the first time
-- they are answered again.

alter table job_time_entries add column if not exists answered_at timestamptz;

-- The answer records itself, and stops claiming to be a stopwatch correction.
...
-- geo_replace_day, unchanged but for the two lines that read the new column:
-- an answer carries the SOURCE across a rebuild and keeps the row out of the
-- sweep, and only fixed_at still carries the TIMES.
