-- ════════════════════════════════════════════════════════════════════════
-- THE SWEEP STOPS WHERE THE DAY STOPS BEING KNOWN (owner 2026-09-18)
--
-- "rebuilt his day... its all duplicative and things arent merged together."
--
-- He was looking at the damage from a guard I added the same morning. On
-- 2026-09-18 p_sweep gained a third condition on both callers, `!res.pending`,
-- because a chain still mid-drive withholds its traced leg (rule 14) and the
-- withheld set was being handed to a SWEEPING write, which retired the good
-- row it had just declined to re-send. Withholding a row must never mean
-- deleting it, and that part was right.
--
-- What was wrong was the blast radius. One unresolved chain at the END of a day
-- turned the sweep off for the WHOLE day, so every stale row from every earlier
-- derive survived and every rebuild added more beside them. Jack's 18th:
--
--   08:00:36-08:27:07  d-j-987ebc83-mu6ym0i3  shop_time_entries   (08:04 derive)
--   08:00:36-08:27:07  d-j-987ebc83-mu6ym0i3  job_time_entries    (08:56 derive)
--
-- The same key, the same 27 minutes, twice, because the dwell changed KIND once
-- the fixes were clean and a dwell that changes kind moves TABLE. The sweep
-- retires anything in the day not in the new key set, so it would have caught
-- that on its own. It never ran. Same for his two drives to Neenans: the live
-- derive saw only the 13:21:46 flip and minted j-...mu7ac5l6; the rebuild had
-- the 13:19:59 flip too (it arrived in a 5-hour-late tape backfill) and minted
-- j-...mu7a9uyp from the earlier start. Two ids for one drive, and with the
-- sweep off, both stood.
--
-- So the rule is the same rule, said with a boundary on it: a derive may retire
-- automatic rows for the part of the day it has an answer for, and must not
-- touch the part it does not. p_sweep_until is that line. Null means the whole
-- day, which is every caller that has no pending chain.
--
-- Rows are judged by where they START. A dwell or drive that began before the
-- line is described by this derive; one that began after it is inside the
