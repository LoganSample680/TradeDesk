-- ════════════════════════════════════════════════════════════════════════
-- "WORKING" RESTORES WHAT THE DERIVER SAID (owner 2026-09-18)
--
-- "how do we have personal rows and neenans going to a fucking onsite versus
-- supply house..?"
--
-- Because answering a row "yes, this was work" relabelled it a CLIENT visit.
-- geo_answer_visit (20261018) has always done this:
--
--   src := case p_mode when 'working' then 'client'
--                      when 'personal' then 'dismissed' else null end;
--
-- One source for every working answer, whatever the row actually was. His
-- 18 September at 17:01:52 he answered the Neenans drive and the Neenans stop
-- working, and a supply house became an onsite. A shop row would have become a
-- client. A home-office row would have become a client.
--
-- geo_answer_supply_run, written two days later, already says the right thing
-- in its own comment: "WORKING RESTORES WHAT THE DERIVER SAID, it does not
-- guess a source." It could hardcode 'place-supply' because it only ever runs
-- on supply runs. The per-row door serves every shape the deriver invents, so
-- it cannot hardcode anything: it has to remember.
--
-- So the row remembers. derived_source is what geo_replace_day last computed,
-- kept beside `source` and never overwritten by an answer. An answer still
-- wins for display, exactly as before; taking the answer back now has
-- something true to go back TO.
-- ════════════════════════════════════════════════════════════════════════

alter table job_time_entries add column if not exists derived_source text;

-- Backfill what can be known without guessing: a row nobody has answered still
-- carries the deriver's own source in `source`. An answered row is left null
-- and refills on its next rebuild, which is one button and costs nothing.
update job_time_entries
   set derived_source = source
 where derived_source is null
   and answered_at is null
   and coalesce(source,'') not in ('', 'manual', 'dismissed', 'client');

comment on column job_time_entries.derived_source is
  'What geo_replace_day last derived for this row. `source` may hold a person''s answer on top; this is what "working" restores.';

-- ── geo_replace_day: record the derived source on every write ──────────────
-- Copied from 20261025 with two lines added, because `create or replace` needs
-- the whole body. Everything else is byte-identical to that migration.
  ...
  for r in select * from jsonb_array_elements(coalesce(p_time,'[]'::jsonb)) loop
    ...
    insert into job_time_entries
      (contractor_user_id, employee_user_id, job_id, arrived_at, departed_at, minutes,
       source, client_key, dest_place, origin_place, deleted_at, derived_source)
    values
      (p_contractor, p_employee, nullif(r->>'job_id',''), lower(sp), upper(sp),
       ...
       r->>'origin_place', null, coalesce(r->>'source','geofence'))
    ...
    do update set
      ...
      -- ALWAYS the deriver's own answer, never the person's. This is the one
      -- column an answer must not reach: it is what "working" goes back to.
...
-- ── geo_answer_visit: working restores, it does not guess ──────────────────
-- The fix this migration exists for. 'personal' is unchanged: a person saying
-- a stop was not work is an answer about the stop, not about its shape, and
-- 'dismissed' is the one source that MEANS "answered, not work".
--
-- 'working' now puts back what the deriver said. The fallback, for a row whose
-- derived_source predates this migration and has not been rebuilt since, is
-- the only thing the key itself can tell us: 'd-' is a dwell and anything else
-- is a drive. That is never worse than 'client' and is usually right; one
-- Rebuild replaces it with the truth.
...
begin
  ...
  if p_mode not in ('working','personal') then
    raise exception 'geo_answer_visit: mode must be working or personal';
  end if;
  if p_mode = 'personal' then
    src := 'dismissed';
  else
    src := coalesce(
      nullif(row.derived_source, ''),
      case when row.client_key like 'd-%' then 'geofence' else 'drive' end);
    -- Never restore INTO an answer: derived_source is only ever the deriver's,
    -- but a legacy row backfilled before this migration could hold one.
    if src = 'dismissed' then
      src := case when row.client_key like 'd-%' then 'geofence' else 'drive' end;
    end if;
  end if;
  update job_time_entries set source = src, answered_at = now() where id = p_id;
  return jsonb_build_object('id', p_id, 'source', src);
end $$;

revoke all on function geo_answer_visit(uuid, text) from public;
