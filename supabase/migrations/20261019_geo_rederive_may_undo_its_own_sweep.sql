-- ── A DERIVER TOMBSTONE HAS TO BE REVERSIBLE (owner 2026-09-16) ───────────
--
-- "Jack's mileage and timesheet is perfect but mine isn't, how can we restore
-- mine?" Then, on finding out how: "why did it permanent delete, nothing
-- should permanent delete, it should all go soft."
--
-- He could not restore them, and neither could a rebuild. The commute sweep
-- that ran while the support view had him marked as crew soft-deleted six of
-- his mileage legs, and prevent_undeletion() makes that permanent: once
-- deleted_at is set, the trigger returns OLD for EVERY later update to that
-- row. So the row is dead, and geo_replace_day cannot write the leg again
-- either, because its id is taken by the corpse. Nineteen and a half miles
-- across four days, gone, with the re-derive silently unable to put them back.
-- Freeing those ids meant a real DELETE run by hand against the database,
-- which is exactly the thing he is right that nothing should ever need.
--
-- The guard itself is right and stays. A record a PERSON deleted must never
-- come back from a stale device, and that is every other td_ table and every
-- client delete path, untouched by this.
--
-- A mileage leg is not that. No person ever deleted these: geo_replace_day
-- wrote them and geo_replace_day retired them, and CLAUDE.md 17 says it is the
-- only writer of an automatic row. The one writer is allowed to take back its
-- own tombstone, and nothing else is: the hatch is a transaction-local setting
-- that ONLY geo_replace_day sets, so it is open for exactly the rows that one
-- function writes inside its own transaction and closed everywhere else. It
-- cannot be set from the client: PostgREST has no way to issue SET LOCAL, and
-- the setting dies with the transaction either way.
--
-- What this buys: a wrong sweep stops being permanent. Re-derive the day and
-- the legs come back on their own ids, which is what "it should all go soft"
-- has to mean if a soft delete is ever to be undone.

create or replace function prevent_undeletion() returns trigger
language plpgsql
as $$
begin
  if OLD.deleted_at is not null
     and coalesce(current_setting('app.geo_rederive', true), '') <> 'on' then
    return OLD;
  end if;
  return NEW;
end;
...
-- geo_replace_day, unchanged but for the one line that opens the hatch.
