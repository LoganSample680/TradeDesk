-- ════════════════════════════════════════════════════════════════════════
-- geo_replace_day: the person's answer survives the rebuild.
--
-- Owner 2026-09-10, watching his Aldi GUYS stop after the rebuild that was
-- meant to fix its minutes: "Why did it flip to visit?"
--
-- Because the rebuild threw away the answer he had already given. The row
-- went from source 'client' to 'client-held', which is rule 13's question,
-- and the reader keeps a held row out of every total and asks about it. His
-- minutes were fixed (14, correctly) and his answer was lost in the same
-- write.
--
-- 6 September 2026 was a SUNDAY. Working days default to Monday through
-- Saturday, so rule 13 does not trust a client dwell that evening without
-- corroboration and marks it a question. That is the rule working. What is
-- broken is that he had ALREADY answered it: naming the address
-- (_mileNameStopRow, js/mileage.js) writes dest_place, sets source to
-- 'client', and stamps fixed_at, which is precisely "a human told us what
-- this was".
--
-- js/geo-derive.js says this is meant to hold, in as many words: "the
-- dashboard asks; geo_replace_day carries the answer (source, under
-- fixed_at) across every rebuild after that." The function never did. The
-- pre-pass above carries the hand-corrected TIMES across, and then the
-- upsert overwrote source and dest_place from the derive regardless.
--
-- Both are carried now, because both are things a person said. dest_place
-- matters for the same reason: a stop he named must not go blank the next
-- time the deriver cannot resolve that fence, which is the whole point of
-- "save this address".
--
-- Also folds in 20260924's rounding, since this recreates the same function
-- and dropping it would silently reintroduce the fraction.
-- ════════════════════════════════════════════════════════════════════════
