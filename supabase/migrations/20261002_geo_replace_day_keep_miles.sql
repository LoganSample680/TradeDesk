...
-- the SAME call rewrote the morning's 3.1-mile leg to 2.9.
--
-- ROOT CAUSE, one sentence: geo_replace_day replaces an existing automatic
-- mileage row's data wholesale with whatever the caller derived, and a server
-- derive has no router and a thinner set of fixes, so it can only ever hand
-- back a smaller number than the phone already worked out.
--
-- Same function as 20261001 with one guard added in the mileage loop and no
-- other line changed. See the comment there for why this covers miles and
-- deliberately does not cover time.
