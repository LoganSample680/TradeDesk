-- RULE 15's SIBLING: A DRIVE ROW NAMES BOTH OF ITS ENDS (owner 2026-09-15)
--
-- "The way it's titled is wrong, we should have fixed the title a long time
-- ago rather than last night."
--
-- A drive row carried only where it ENDED, so the Time Log had to join to the
-- mileage leg and work out which segment it was looking at just to say "shop
-- to Bill Lorson". Every failure of that title came out of the join, not out
-- of the data: a collapsed leg's from_name is the whole journey's, a day
-- derived before segEnds existed has nothing to read at all (Jack's 14
-- September, every drive titled only by its destination), and a screen holding
-- time rows without mileage rows can never title them.
--
-- origin_place is the mirror of dest_place and is written by the same deriver
-- pass (geoDeriveRows, js/geo-derive.js). Additive and nullable, so production
-- code that has never heard of it reads these rows exactly as before
-- (CLAUDE.md 3.1). It takes the same fixed_at guard dest_place has: a person's
-- answer is never talked over by a re-derive.

alter table job_time_entries add column if not exists origin_place text;

-- A held drive keeps its answer.
--
-- Owner 2026-09-12: "Jack didn't work Thursday or Friday this last week why do
-- we have mileage and time rows in there?" Rule 13 had asked that question
-- about visits since 2026-09-04 and never once about the drives between them,
-- so a held visit still produced claimed business miles either side of it.
-- Rule 15 (js/geo-derive.js _gdHeldLegs) now holds a drive with no business
-- end, and this is the half that makes the hold answerable.
--
-- The deriver is pure and re-sets the hold on every rebuild, so an answer
-- stored nowhere else is wiped the next time the day derives. That is exactly
-- what happened to the receipt hold on 2026-09-05 and this is the same fix in
-- the same place: same function as 20261002, four lines added to the mileage
-- merge, nothing else touched.
