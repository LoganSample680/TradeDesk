-- ════════════════════════════════════════════════════════════════════════
-- geo_replace_day: a minute is a whole number.
--
-- Owner 2026-09-10, looking at his own Time Log: "Why did this happen to
-- Aldi guys??" The row read "14.2700016666666667m", and the fraction then
-- travelled into every total that sums it: the day read
-- "2h 24.2700166666666663m", the week "32h 57.2700166666666606m", and the
-- on-site subtotal "25h 44.270016666666606m".
--
-- ROOT CAUSE: this function recomputes `minutes` for a row that has been
-- hand-fixed, and did it as extract(epoch from ...)/60 with no rounding.
-- Every other writer rounds. js/geo-derive.js rounds at _gdDwell and at
-- every leg; js/timelog.js rounds in the edit form. So the only rows that
-- could carry a fraction were the ones a person had touched, which is why
-- it took a hand-named stop to surface a bug that has been in every
-- re-derive since 20260909.
--
-- His Aldi GUYS stop ran 16:46:37.298 to 17:00:53.499, which is 14 minutes
-- and 16 seconds. The deriver wrote 14. He named the address, which stamps
-- fixed_at, and the next rebuild recomputed 14.2700166666666667 and kept it.
--
-- Fixed in four places, not one: both hand-fixed recompute branches (time
-- and shop) and both insert fallbacks, which do the same arithmetic and had
-- the same omission. Patching only the branch that bit would have left the
-- identical bug in three places for the next person to find.
--
-- NO DATA REPAIR NEEDED, and that is the design working: the row is
-- fixed_at, so the next boot rebuild recomputes it through this function and
-- the fraction is gone. The deriver is the one writer (CLAUDE.md 17); a
-- hand-written UPDATE here would be a second one.
-- ════════════════════════════════════════════════════════════════════════
