-- ── "Not work" reaches a shop row too (owner 2026-09-13) ───────────────────
--
-- 20261011 widened geo_answer_visit so anything automatic can be marked
-- personal, for the case the owner named: "there are times he could go to his
-- dads shop and it not be work related, just visiting his old man."
--
-- That fixed the wrong table. The shop dwell he was describing is not in
-- job_time_entries at all, it is in shop_time_entries, and geo_answer_visit
-- only ever looks at the former. So the one row the ask was ABOUT was the one
-- row that still could not be answered.
--
-- WHY THIS IS A SECOND FUNCTION rather than a branch in the first: the two
-- tables say "this did not count" in two different ways, because they are
-- shaped differently. job_time_entries carries a `source` column, so an
-- answered visit becomes source='dismissed' and stays a row. shop_time_entries
-- has no source column at all (it has never needed one: every row in it means
-- the same thing), so the only way to say it is soft deletion. One function
-- with an if-shop-then-completely-different-update inside it would be two
-- functions wearing one name.
--
-- THE HARD PART IS NOT THE DELETE, IT IS MAKING IT STICK. geo_replace_day's
-- shop upsert ends `deleted_at = null` on conflict, deliberately, so a row
-- that came back on a re-derive is live again. The tape has not changed, so
-- the deriver still produces that dwell tomorrow and every day after, and a
-- plain delete would reappear within the hour. That is exactly the "why did it
-- come back" bug this whole line of work exists to end.
--
-- fixed_at is already the flag that means A PERSON DECIDED THIS. The sweep has
-- spared fixed_at rows since 20260909 ("what a person wrote or corrected is
-- never retired by a rebuild"). This adds the mirror of that sentence to the
-- upsert: a row a person deleted is never RESURRECTED by one either. Both
-- halves are the same rule, and neither is true without the other.
--
-- ANSWERING IS REVERSIBLE, in the data if not yet in the UI: 'working' clears
-- deleted_at and leaves fixed_at set, so the row comes back and stays pinned.

create or replace function geo_answer_shop(p_id uuid, p_mode text)
returns jsonb
...
declare
  ...
  row shop_time_entries%rowtype;
begin
  if me is null then raise exception 'geo_answer_shop: not signed in'; end if;
  select * into row from shop_time_entries where id = p_id;
  if row.id is null then raise exception 'geo_answer_shop: no such visit'; end if;
  -- Same check geo_answer_visit and geo_replace_day both make: the row's own
  -- person, or the account that owns it.
  if me <> row.employee_user_id and me <> row.contractor_user_id then
    raise exception 'geo_answer_shop: not your visit';
  end if;
  if p_mode = 'personal' then
    update shop_time_entries set deleted_at = now(), fixed_at = now() where id = p_id;
  elsif p_mode = 'working' then
    update shop_time_entries set deleted_at = null, fixed_at = now() where id = p_id;
  else
    raise exception 'geo_answer_shop: mode must be working or personal';
  end if;
  return jsonb_build_object('id', p_id, 'mode', p_mode);
end $$;

grant execute on function geo_answer_shop(uuid, text) to authenticated;

-- ── And the upsert stops undoing it ────────────────────────────────────────
-- Byte-for-byte 20261004 (verified against the live function's own source)
-- with exactly one line changed, marked below. Every other behaviour it has
-- accumulated since 20260906 (the overlap refusal, the fixed_at span and
-- minutes rules, the kept source, the receipt fields, the rounding, the
-- service_role grant) is carried through untouched.
...
begin
  ...
  for r in select * from jsonb_array_elements(coalesce(p_shop,'[]'::jsonb)) loop
    ...
    do update set
      ...
      -- THE ONE CHANGED LINE, and it used to be an unconditional null. A row
      -- somebody answered "not work" (geo_answer_shop above stamps fixed_at
      -- AND deleted_at) stays gone. The tape has not changed, so without this
      -- the deriver hands that dwell straight back on the next rebuild, which
      -- is the "why did it come back" bug this line of work exists to end.
      -- Anything deleted by something OTHER than an answer still comes back
      -- live exactly as before, because only an answer sets fixed_at.
      deleted_at  = case when shop_time_entries.fixed_at is not null
                          and shop_time_entries.deleted_at is not null
                         then shop_time_entries.deleted_at else null end;
