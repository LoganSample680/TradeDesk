-- ════════════════════════════════════════════════════════════════════════
-- THE WRITER LEARNS AN OPEN ROW (owner 2026-09-18)
--
...
--
-- geoDeriveRows started returning the open dwell (js/geo-derive.js), shaped
-- like any other row with departed_at and minutes null, because the arrival is
-- a fact the moment it happens and storing it only once the person leaves is
-- what kept every timesheet one event behind the truck.
--
-- This is the writer accepting it. Three things had to be true and only the
-- first needed new SQL:
--
--   1. The overlap invariant pairs every incoming row against every other.
--      An open row runs to infinity, so it overlaps everything after it, and
--      pairing it would refuse every day that ends with somebody still
--      standing somewhere. It is now checked for the only things that can
--      really be wrong: at most one, and never beginning before a closed row
--      ends.
--   2. minutes already lands NULL on its own. tstzrange(a, null) is unbounded,
--      upper() of it is null, and the arithmetic that would have computed
--      minutes returns null rather than raising. Nothing to change.
--   3. The sweep already spares it, because the open row is in the incoming
--      key list like any other.
--
-- And one thing that was NOT already safe: a hand-corrected row must never be
-- re-opened. A null departure landing on a row somebody fixed by hand would
-- erase their end time and their minutes in a single write, so both upserts
-- now keep the person's values when the incoming departure is empty.
--
-- CLOSING IS AN UPDATE, NOT A SECOND ROW. The open row carries the same
-- client_key the closed row will carry, because the id is minted from the
-- CoreMotion flip, so driving away upserts over it.
-- ════════════════════════════════════════════════════════════════════════
