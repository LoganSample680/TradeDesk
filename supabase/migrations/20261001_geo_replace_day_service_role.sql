-- The server may write a day too.
--
-- Owner 2026-09-11: "why not just point it easily server side so it doesnt need
-- a app active to run it, server runs it as they happen, in real-time."
--
-- ingest-geo now runs the real deriver (js/geo-derive.js, generated into
-- supabase/functions/_shared/geo-derive.mjs) over the events in the batch it
-- just stored, and writes through this function, which is still the ONLY writer
-- of automatic rows (CLAUDE.md 17). Nothing about WHAT it writes changes. The
-- one thing that had to change is who may call it: an edge function holds the
-- service role and therefore has no auth.uid(), so every server derive came
-- back 'geo_replace_day: not signed in'.
--
-- This is the same function as 20260925 with a declare and a guard added and
-- not one other line touched, so the merge rules, the overlap refusal, the
-- fixed_at preservation and the p_sweep semantics are all exactly as they were.
--
-- WHY THIS IS NOT A HOLE: the service role key lives in edge function
-- environments and nowhere else, never in the app bundle and never in a
-- browser. Anything holding it can already read and write every table
-- directly, so routing it through this function means it goes THROUGH the
-- overlap guard and the preservation rules rather than around them, which is
-- strictly safer than the alternative.
--
-- The server always passes p_sweep false (supabase/functions/_shared/
-- derive-day.mjs): it only ever sees what has been flushed to it, so absence of
-- evidence there is not evidence of absence. Only the phone, whose own
-- CoreMotion tape covers the day end to end, may retire a row.
