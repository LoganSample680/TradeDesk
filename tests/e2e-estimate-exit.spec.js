...
test.describe('leaving a T&M or BYO estimate', () => {
  ...
  // Same panel, same exits, on both editors (owner: "What about BYO? Is it
  // plagued by the same problem?"). BYO talks into its own box, byo-say.
  const talk = (heard, mode) => page.evaluate(({ heard, mode }) => {
  ...
  for (const mode of ['tm', 'byo']) {
    test(`${mode}: the listening panel sits above the bottom bar: Done talking is on top and on screen`, async () => {
      await open(mode, mode === 'tm' ? 99106 : 99116);
      await page.waitForTimeout(500);
    ...
    test(`${mode}: Save while Tim is listening: the mic goes off, the words are kept, then home`, async () => {
      await open(mode, mode === 'tm' ? 99107 : 99117);
      await page.waitForTimeout(500);
      await talk('Replace the kitchen faucet', mode);
      await page.waitForTimeout(200);
      expect(await page.evaluate(() => _timTalking)).toBe(true);
      await page.locator(`#gei-${mode}-page .ios-nav .ios-navbtn.bold`).click();
      await page.waitForTimeout(700);
      const r = await page.evaluate((id) => ({
        talking: _timTalking, mic: window.__micOn, panel: !!document.getElementById('_tim-listen'),
        said: document.getElementById(id)?.value || '', pg: document.querySelector('.pg.active')?.id,
      }), box[mode]);
    ...
    test(`${mode}: Back, or leaving by any page change, turns the mic off too`, async () => {
      await open(mode, mode === 'tm' ? 99108 : 99118);
      await page.waitForTimeout(500);
      await talk('Replace the kitchen faucet', mode);
      await page.waitForTimeout(200);
      await page.locator(`#gei-${mode}-page .ios-nav .ios-navbtn`).first().click();
      ...
      await open(mode, mode === 'tm' ? 99109 : 99119);
      await page.waitForTimeout(500);
      await talk('Replace the kitchen faucet', mode);
      await page.waitForTimeout(200);
