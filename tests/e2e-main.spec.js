...
// ════════════════════════════════════════════════════════════════════════════
//  checkNewSignatures — pre-registration row guard
//
//  Regression: proposals.js inserts a signed_proposals row with signed_at=null
//  when a bid is sent. checkNewSignatures() must ignore that row and, if the
//  bid was previously auto-marked Closed Won by the old bug, reset it to Pending.
//
//  The Supabase shim's from() is a no-op stub (never hits the network).
//  We control what it returns via window.__mockFromTable, which the shim
//  checks at call-time before falling back to queryBuilder().
// ════════════════════════════════════════════════════════════════════════════
test.describe('checkNewSignatures — pre-registration row guard', () => {
  let page;

  test.beforeAll(async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 390, height: 844 }, bypassCSP: true });
    page = await ctx.newPage();
    await mockAllExternal(page, { alreadySigned: false });
    await page.goto('/', { waitUntil: 'domcontentloaded', timeout: 20000 });
    await waitForAppBoot(page);
  });
  ...
  /**
   * Set up the signed_proposals mock, inject bid at given status, run
   * checkNewSignatures(), return resulting bid status.
   */
  async function runCheck(sigRow, bidStatus) {
    return page.evaluate(async ([uid, bid1, row, status]) => {
      window._supaUser = { id: uid, email: 'test@test.com' };
      localStorage.removeItem('zp3_seen_sigs');
      // Inject or update the bid
      if (typeof bids !== 'undefined') {
        const ex = bids.find(b => String(b.id) === String(bid1));
        if (ex) { ex.status = status; ex.signedAt = status === 'Closed Won' ? '2026-01-01T00:00:00Z' : null; }
        else { bids.push({ id: bid1, status, client_id: '901', amount: 1000 }); }
      }
      // Override _supa.from('signed_proposals') via shim hook
      window.__mockFromTable = {
        signed_proposals: () => {
          const data = row ? [row] : [];
          const q = {
      ...
      try { if (typeof checkNewSignatures === 'function') await checkNewSignatures(); } catch (_) {}
      delete window.__mockFromTable;
      if (typeof bids === 'undefined') return null;
      const b = bids.find(b => String(b.id) === String(bid1));
      return b ? b.status : null;
    }, [FAKE_USER_ID, FAKE_BID_ID_1, sigRow, bidStatus]);
  }

  test('pre-registration row (signed_at=null) does NOT mark Pending bid Closed Won', async () => {
    const status = await runCheck(
      { bid_id: String(FAKE_BID_ID_1), client_name: null, client_signed_name: null,
        payment_method: null, payment_status: null, signed_at: null },
      'Pending'
    );
    expect(status).toBe('Pending');
  });

  test('pre-registration row (signed_at=null) resets corrupted Closed Won bid to Pending', async () => {
    const status = await runCheck(
      { bid_id: String(FAKE_BID_ID_1), client_name: null, client_signed_name: null,
        payment_method: null, payment_status: null, signed_at: null },
      'Closed Won'
  ...
  test('real signed row (signed_at set) marks Pending bid Closed Won', async () => {
    const status = await runCheck(
      { bid_id: String(FAKE_BID_ID_1), client_name: 'Alice Smith', client_signed_name: 'Alice Smith',
        payment_method: 'cash', payment_status: 'pending_cash', signed_at: new Date().toISOString() },
      'Pending'
    );
    expect(status).toBe('Closed Won');
  });

  test('no console errors during pre-registration check', async () => {
    await runCheck(
      { bid_id: String(FAKE_BID_ID_1), client_name: null, client_signed_name: null,
        payment_method: null, payment_status: null, signed_at: null },
      'Pending'
    );
    assertNoErrors(page, 'checkNewSignatures pre-registration guard');
  });
});
