...
function addVehicle(){
  ...
  const nickInp=document.getElementById('set-new-veh-nick');
  const name=(inp?inp.value.trim():'');
  const nickname=(nickInp?nickInp.value.trim():'');
  if(!name)return zAlert('Enter a vehicle description (e.g. 2019 White F-150)');
  const vehs=getVehicles();
  if(vehs.find(v=>v.name===name))return zAlert('Vehicle already added.');
  vehs.push({name,nickname,addedDate:todayKey()});
  S.vehicles=vehs;
  saveAll();
  if(inp)inp.value='';
  if(nickInp)nickInp.value='';
  renderVehicleSettings();
  setTimeout(()=>_checkOdometerPrompt(),400);
}
...
function renderVehicleSettings(){
  const el=document.getElementById('set-vehicles-list');if(!el)return;
  const vehs=getVehicles();
  if(!vehs.length){el.innerHTML='<div style="font-size:12px;color:var(--text3);margin-bottom:4px">No vehicles added yet.</div>';return;}
  el.innerHTML=vehs.map((v,i)=>{
    const label=getVehicleLabel(v);
    const sub=v.nickname&&v.nickname.trim()?v.name:'';
    const nickVal=v.nickname||'';
    return '<div style="background:var(--bg2);border-radius:var(--r);padding:8px 12px;margin-bottom:4px">'+
      '<div style="display:flex;justify-content:space-between;align-items:center">'+
        '<div style="font-size:13px;font-weight:700">'+label+'</div>'+
        '<button class="btn-del" onclick="removeVehicle('+i+')" style="font-size:11px;padding:3px 8px">Remove</button>'+
      '</div>'+
      ...
      '<select data-gvwr-idx="'+i+'" oninput="updateVehicleGVWR('+i+',this.value)" style="margin-top:4px;font-size:11px;padding:4px 8px;border-radius:var(--r);border:1px solid var(--border2);background:var(--bg);color:var(--text);width:100%;box-sizing:border-box;font-family:inherit">'+
        ...
        '<option value="heavy_truck">Over 6,000 lbs — Pickup or Van (F-150, RAM, Sprinter)</option>'+
        '<option value="heavy_suv">Over 6,000 lbs — Large SUV (Tahoe, Expedition, Suburban)</option>'+
        '<option value="commercial">Over 14,000 lbs (box truck, flatbed, commercial)</option>'+
      '</select>'+
      _gvwrNote(v.gvwr||'')+
    '</div>';
  }).join('');
  // Set input values after innerHTML (avoids quote issues)
  vehs.forEach((v,i)=>{
    const inp=el.querySelector('[data-veh-idx="'+i+'"]');
    if(inp)inp.value=v.nickname||'';
    const gvwrSel=el.querySelector('[data-gvwr-idx="'+i+'"]');
    if(gvwrSel)gvwrSel.value=v.gvwr||'';
  });
}
function updateVehicleNick(idx,nick){
  const vehs=getVehicles();
  if(vehs[idx]){vehs[idx].nickname=nick;S.vehicles=vehs;saveAll();}
}
function updateVehicleGVWR(idx,val){
  const vehs=getVehicles();
  if(vehs[idx]){vehs[idx].gvwr=val;S.vehicles=vehs;saveAll();renderVehicleSettings();}
}
function _gvwrNote(gvwr){
  if(gvwr==='light')return '<div style="font-size:11px;background:#FEF3C7;border:1px solid #D97706;border-radius:var(--r);padding:6px 8px;margin-top:5px;color:#92400E">⚠️ <strong>Section 280F applies:</strong> first-year depreciation capped ~$12,200. Standard mileage rate often beats actual expenses for these vehicles.</div>';
  if(gvwr==='heavy_truck')return '<div style="font-size:11px;background:#F0FDF4;border:1px solid #16A34A;border-radius:var(--r);padding:6px 8px;margin-top:5px;color:#166534">✓ <strong>No 280F limits.</strong> Full Section 179 or bonus depreciation available (up to $70,000 in 2025). Keep mileage log proving &gt;50% business use every year.</div>';
  if(gvwr==='heavy_suv')return '<div style="font-size:11px;background:#FEF3C7;border:1px solid #D97706;border-radius:var(--r);padding:6px 8px;margin-top:5px;color:#92400E">⚠️ <strong>Section 179 SUV cap:</strong> max $31,300 in 2025. No luxury auto limits, but the SUV cap applies. A pickup truck with a bed doesn\'t have this cap.</div>';
  if(gvwr==='commercial')return '<div style="font-size:11px;background:#F0FDF4;border:1px solid #16A34A;border-radius:var(--r);padding:6px 8px;margin-top:5px;color:#166534">✓ <strong>Commercial vehicle:</strong> no Section 280F limits. Full Section 179 deductible. Maintain &gt;50% business use documentation.</div>';
  return '<div style="font-size:10px;color:var(--text3);margin-top:4px">Set weight class above — determines how much depreciation you can deduct (IRS §280F).</div>';
}
// ══════════════════════════════════════════════════════════════════
