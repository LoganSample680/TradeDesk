...
async function sendGenericProposal(previewOnly){
  ...
  const _tmDepRow=`<tr style="background:${_geiIsTM?'#0369a1':'#2a4a7f'};color:rgba(255,255,255,.88)"><td colspan="2" style="padding:6px 18px;font-size:11px;font-weight:600">${_geiIsTM?`Mobilization Deposit (${_tmDepPct}%)`:`${_tmDepPct}% Deposit`} Due Before Work Begins</td><td style="padding:6px 18px;text-align:right;font-size:12px;font-weight:700;white-space:nowrap">${depositFmt}</td></tr>`;
  ...
  if(typeof calcSalesTax==='function'&&_stRateP>0){
    ...
    if(_stRes.treatment.customerTax&&_stRes.taxAmount>0){
      ...
      _stRowHtml=`<tr style="border-bottom:1px solid #e2e8f0;background:#f8fafc"><td colspan="2" style="padding:8px 18px;font-size:12px;color:#64748b">${_stLblP}</td><td style="padding:8px 18px;text-align:right;font-size:12px;color:#64748b;white-space:nowrap">$${_stRes.taxAmount.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>`;
    } else if(_stScopeP==='improvement'&&!_stRes.treatment.customerTax){
      _stRowHtml=`<tr style="border-bottom:1px solid #e2e8f0;background:#f8fafc"><td colspan="2" style="padding:8px 18px;font-size:12px;color:#64748b">New construction: no tax</td><td style="padding:8px 18px;text-align:right;font-size:12px;color:#64748b;white-space:nowrap">$0.00</td></tr>`;
    }
  }
  ...
  const _nteRow=(_geiIsTM&&_tmNteCap)?`<tr style="background:#075985;color:rgba(255,255,255,.8)"><td colspan="2" style="padding:5px 18px;font-size:11px">Not-to-exceed cap</td><td style="padding:5px 18px;text-align:right;font-size:11px;font-weight:700;white-space:nowrap">$${_tmNteCap.toLocaleString()}</td></tr>`:'';
