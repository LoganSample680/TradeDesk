...
function _cdPropCardHtml(c,a,idx,total){
  ...
  if(isOpen){
    ...
    const _photoBlock=`<div style="display:flex;align-items:center;gap:10px;margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
      ...
      <button onclick="event.stopPropagation();tdImportPhotos(${c.id},${escHtml(JSON.stringify(a.addr||''))})" style="background:none;border:1px solid var(--border2);border-radius:var(--r);padding:7px 12px;font-size:12px;font-weight:800;cursor:pointer;font-family:inherit;color:var(--blue)">Import</button>
